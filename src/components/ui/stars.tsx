...
type StarsBackgroundProps = React.ComponentProps<"div"> & {
  ...
  starColor?: string;
  /** Track window mouse (for full-site bg without blocking clicks) */
  globalMouse?: boolean;
};
...
export function StarsBackground({
  ...
  const applyMouse = React.useCallback(
  ...
  const handleMouseMove = React.useCallback(
    ...
    [applyMouse, globalMouse]
  );

  React.useEffect(() => {
    if (!globalMouse) return;
    const onMove = (e: MouseEvent) => applyMouse(e.clientX, e.clientY);
    window.addEventListener("mousemove", onMove, { passive: true });
    return () => window.removeEventListener("mousemove", onMove);
  }, [globalMouse, applyMouse]);
